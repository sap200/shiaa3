...
async function makeClientWithAuth(rpcUrl) {
    ...
    return [queryclient_1.QueryClient.withExtensions(tmClient, auth_1.setupAuthExtension), tmClient];
}
describe("AuthExtension", () => {
    describe("account", () => {
        ...
        it("works for account with pubkey and non-zero sequence", async () => {
            ...
            utils_1.assert(account);
            const pubkey = proto_signing_1.encodePubkey(testutils_spec_1.validator.pubkey);
            expect(account).toEqual({
                address: testutils_spec_1.validator.delegatorAddress,
                pubKey: any_1.Any.fromPartial(pubkey),
        ...
        it("returns null for non-existent address", async () => {
            testutils_spec_1.pendingWithoutSimapp();
            const [client, tmClient] = await makeClientWithAuth(testutils_spec_1.simapp.tendermintUrl);
            const account = await client.auth.account(testutils_spec_1.nonExistentAddress);
    ...
    describe("unverified", () => {
        describe("account", () => {
            it("works for unused account", async () => {
                testutils_spec_1.pendingWithoutSimapp();
                const [client, tmClient] = await makeClientWithAuth(testutils_spec_1.simapp.tendermintUrl);
                const account = await client.auth.unverified.account(testutils_spec_1.unused.address);
                utils_1.assert(account);
                expect(account).toEqual({
                    address: testutils_spec_1.unused.address,
                    // pubKey not set
            ...
            it("works for account with pubkey and non-zero sequence", async () => {
                testutils_spec_1.pendingWithoutSimapp();
                const [client, tmClient] = await makeClientWithAuth(testutils_spec_1.simapp.tendermintUrl);
                const account = await client.auth.unverified.account(testutils_spec_1.validator.delegatorAddress);
                utils_1.assert(account);
                const pubkey = proto_signing_1.encodePubkey(testutils_spec_1.validator.pubkey);
                expect(account).toEqual({
                    address: testutils_spec_1.validator.delegatorAddress,
            ...
            it("returns null for non-existent address", async () => {
                testutils_spec_1.pendingWithoutSimapp();
                const [client, tmClient] = await makeClientWithAuth(testutils_spec_1.simapp.tendermintUrl);
                await expectAsync(client.auth.unverified.account(testutils_spec_1.nonExistentAddress)).toBeRejectedWithError(/account cosmos1p79apjaufyphcmsn4g07cynqf0wyjuezqu84hd not found/i);
                tmClient.disconnect();
            });
        });
    });
